...
$(document).on('click', '.fa-shopping-cart', function (e) {
  ...
  if (existingCartItem) {
    ...
    $.post("http://meteo-shop/setCart", JSON.stringify({
      type: type,
      price: price,
      image: image,
      id: id,
...
$(document).on('click', '.pay', function (e) {
  ...
  $.post("http://meteo-shop/pay", JSON.stringify({price:totalCartPrice,payType: payType, cartArr: cartArr}));
  closeNUI();
})
...
function closeNUI() {
  $("body").hide()
  $.post("http://meteo-shop/close", JSON.stringify({}));
  $(".job-box,.job-cart-circle").animate({
    top: "15rem"
  }, 500)
}
