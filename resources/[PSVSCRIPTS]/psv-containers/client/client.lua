...
RegisterNetEvent("psv-containers:client:notification", function( msg, type ) Notification_c(msg, type) end)
    ...
    for k, v in pairs(Config.container_depots) do
        ...
        ZONE[k]:onPlayerInOut(function( isPointInside )
            if isPointInside then
                ...
                Wait(50)
                ShowDrawText(v.name)
                TriggerCallback("psv-containers:server:GET:ZONE:containers", function( containers ) for k, container in pairs(containers) do Containers:new(container) end end, current_zone)
            else
                current_zone = nil
                Containers:clean_up()
                HideDrawText()
...
RegisterNetEvent("psv-containers:client:update_zone", function( zone_name )
    local current_zone, zone = GetCurrentZone()
    if zone_name == current_zone then
        Containers:clean_up()
        TriggerCallback("psv-containers:server:GET:ZONE:containers", function( containers ) for k, container in pairs(containers) do Containers:new(container) end end, zone_name)
    end
end)
