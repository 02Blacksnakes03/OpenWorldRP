...
RegisterNetEvent("psv-containers:client:container:place", function( container_type )
    ...
    if Config.input ~= "ox_lib" then
        ...
        if inputData and inputData.password and inputData.password ~= "" then
            ...
            if is_in_zone then
                TriggerServerEvent("psv-containers:server:create_container", inputData.password, position, zone_name)
            else
        ...
        if inputData and inputData[1] then
            ...
            local is_in_zone = Zone:isPointInside(position) -- this should be somehow server-side
            if is_in_zone then
                TriggerServerEvent("psv-containers:server:create_container", inputData[1], position, zone_name)
            else
                Notification_c("The container is outside of depot!", "error")
            end
        end
...
AddEventHandler("psv-containers:client:container:update_location", function( random_id, zone_name, container_type )
    ...
    local position = ChooseSpawnLocation(container.object.name, container.object.offset)
    if position == "exit" then
        TriggerEvent("psv-containers:client:update_zone", zone_name)
        return
    end
    local is_in_zone = Zone:isPointInside(position) -- this should be somehow server-side
    if is_in_zone then
        TriggerServerEvent("psv-containers:server:container:update_position", random_id, zone_name, position)
    else
        TriggerEvent("psv-containers:client:update_zone", zone_name)
        Notification_c("The container is outside of depot!", "error")
    end
