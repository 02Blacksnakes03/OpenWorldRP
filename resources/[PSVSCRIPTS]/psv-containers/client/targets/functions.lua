function MoveContainer( private, entity )
    ...
    if Framework == 1 then
        ...
        if confData and confData.conf == "Confirm" then
            ...
            TriggerEvent("psv-containers:client:container:update_location", private.random_id, zone_name, private.container_type)
            DeleteEntity(entity)
        end
...
function DeleteContainer( private, entity )
    ...
    if Framework == 1 then
        ...
        if confData and confData.conf == "Confirm" then TriggerServerEvent("psv-containers:server:container:delete", private.random_id, zone_name) end
    elseif Framework == 2 then
        if Config.input ~= "ox_lib" then
            local confData = exports[Config.input]:ShowInput({
                inputs = {
            ...
            if inputData and inputData[1] == "Confirm" then TriggerServerEvent("psv-containers:server:container:delete", private.random_id, zone_name) end
        end
    end
end
...
function TransferOwnership( private, entity )
    ...
    if Framework == 1 then
        ...
        if inputData and inputData.new_owner then
            ...
            if confData and confData.conf == "Confirm" then TriggerServerEvent("psv-containers:server:container:transfer_ownership", private.random_id, zone_name, inputData.new_owner) end
        end
    elseif Framework == 2 then
        if Config.input ~= "ox_lib" then
            local inputData = exports[Config.input]:ShowInput({
            ...
            if inputData and inputData.new_owner then
                ...
                if confData and confData.conf == "Confirm" then
                    TriggerServerEvent("psv-containers:server:container:transfer_ownership", private.random_id, zone_name, inputData.new_owner)
                end
            end
        else
            ...
            if inputData and inputData[1] then
                ...
                if alert == "confirm" then
                    TriggerServerEvent("psv-containers:server:container:transfer_ownership", private.random_id, zone_name, inputData[1])
                else
...
function ChangePassword( private, entity )
    ...
    if Framework == 1 then
        ...
        if inputData and inputData.current_password and inputData.new_password and (zone_name ~= -1) then
            TriggerServerEvent("psv-containers:server:container:change_password", private.random_id, inputData.current_password, inputData.new_password, zone_name)
        end
    elseif Framework == 2 then
        if Config.input ~= "ox_lib" then
            local Input = {
            ...
            if inputData and inputData[1] and inputData[2] then
                TriggerServerEvent("psv-containers:server:container:change_password", private.random_id, inputData[1], inputData[2], zone_name)
            end
        end
    end
...
function OpenContainer( private, entity )
    ...
    if Framework == 1 then
        ...
        local inputData = exports[Config.input]:ShowInput(Input)
        if inputData and inputData.password then TriggerServerEvent("psv-containers:server:container:check_password", private.random_id, inputData.password, zone_name) end
    elseif Framework == 2 then
        if Config.input ~= "ox_lib" then
            local Input = {
            ...
            if inputData and inputData[1] then TriggerServerEvent("psv-containers:server:container:check_password", private.random_id, inputData[1], zone_name) end
        end
    end
end
...
local function bolt_cutter( entity, random_id, zone_name )
    ...
    TriggerServerEvent("psv-containers:server:open_with_bolt_cutter", random_id, zone_name)
end

RegisterNetEvent("psv-containers:targets:use_bolt_cutter", function( entity, random_id, zone_name ) bolt_cutter(entity, random_id, zone_name) end)
...
function BoltCutter( private, entity )
    ...
    TriggerServerEvent("psv-containers:server:use_bolt_cutter", entity, private.random_id, zone_name)
end
