...
RegisterNetEvent("psv-containers:server:create_container", function( password, position, zone_name )
    ...
    MySQL.Async.insert(sqlQuery, QueryData, function()
        ...
        TriggerClientEvent("psv-containers:client:update_zone", -1, zone_name)
        Notification_S(src, "Success", "success")
    end)
end)

CreateCallback("psv-containers:server:GET:ZONE:containers", function( source, cb, zone_name )
...
RegisterNetEvent("psv-containers:server:container:check_password", function( random_id, password, zone_name )
    local src = source
    MySQL.Async.fetchAll("SELECT container_type, password FROM nchub_containers WHERE random_id = ?", {
        ...
        if VerifyPassword(src, password, res.password, true) == true then
            if Framework == 1 then
                TriggerClientEvent("psv-containers:client:open", src, container_type.type)
            elseif Framework == 2 then
...
RegisterNetEvent("psv-containers:server:use_bolt_cutter", function( entity, random_id, zone_name )
    local src = source
    local player = Player(src)
    ...
    TriggerClientEvent("psv-containers:targets:use_bolt_cutter", src, entity, random_id, zone_name)
end)

RegisterNetEvent("psv-containers:server:open_with_bolt_cutter", function( random_id, zone_name )
    local src = source
    local player = Player(src)
    if not HasAccessToBoltCutter(src) then
        Notification_S(src, "You can't use boltcutter", "error")
...
RegisterNetEvent("psv-containers:server:container:change_password", function( random_id, current_password, new_password, zone_name )
    local src = source
    local Player = Player(src)
    local citizenid = GetCitizenId(Player)
...
RegisterNetEvent("psv-containers:server:container:transfer_ownership", function( random_id, zone_name, new_owner )
    local src = source
...
RegisterNetEvent("psv-containers:server:container:delete", function( random_id, zone_name )
    local src = source
    local player = Player(src)
    local citizenid = GetCitizenId(player)
...
RegisterNetEvent("psv-containers:server:container:update_position", function( random_id, zone_name, new_position )
    local src = source
    local player = Player(src)
    local citizenid = GetCitizenId(player)
    ...
    if not is_super_user(citizenid) then
        Notification_S(src, "Hmm, you can't do that!", "primary")
        TriggerClientEvent("psv-containers:client:update_zone", src, zone_name)
        return
    end
...
if Framework == 1 then
    for k, v in pairs(GetContainerItems()) do
        Core.Functions.CreateUseableItem(k, function( source, item )
            ...
            TriggerClientEvent("psv-containers:client:container:place", source, k)
        end)
    end
elseif Framework == 2 then
    for k, v in pairs(GetContainerItems()) do
        Core.RegisterUsableItem(k, function( playerId )
            ...
            TriggerClientEvent("psv-containers:client:container:place", playerId, k)
        end)
    end
end
