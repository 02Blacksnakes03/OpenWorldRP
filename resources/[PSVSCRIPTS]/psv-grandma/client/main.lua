    ...
    while true do
        ...
        if dist <= 1.5 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                TriggerServerEvent("psv-grandma:server:hug", spawn)
                Citizen.Wait(5000)
                FreezeEntityPosition(ped, false)
                TriggerServerEvent("psv-grandma:server:idle", spawn)
                TriggerServerEvent('psv-grandma:server:SetResourceCooldown')
            end
        else
            Citizen.Wait(5000)
        end
            ...
            options = {
                {
                    num = 1,
                    type = "client",
                    event = "psv-grandma:client:MedicalAidInjuries",
                ...
                },
                {
                    num = 2,
                    type = "client",
                    event = 'psv-grandma:client:MedicalAid',
...
RegisterNetEvent('psv-grandma:client:hug', function(coords)
    local model = GetClosestHug(coords)
    TaskPlayAnim(model, "mp_ped_interaction", "kisses_guy_a", 8.00, -8.00, 5000, 51, 0.00, 0, 0, 0)
end)

RegisterNetEvent('psv-grandma:client:idle', function(coords)
    local model = GetClosestHug(coords)
    TaskPlayAnim(model, "amb@world_human_aa_smoke@male@idle_a", "idle_c", 8.00, -8.00, -1, 51, 0.00, 0, 0, 0)
end)

RegisterNetEvent("psv-grandma:client:MedicalAid", function()
        ...
        if Config.IllegalMedical.Minigame.Enabled == true then
            if Config.IllegalMedical.Minigame.PSUI then
                exports['ps-ui']:Circle(function(success)
                    if success then
                    TriggerServerEvent("psv-grandma:server:MedicalAid")
                    else
...
RegisterNetEvent("psv-grandma:client:MedicalAidInjuries", function()
    QBCore.Functions.Progressbar('ConstantDevelopmentIllegalGrandmaMedical', Config.IllegalMedical.PedName..' is helping you..', math.random(2500, 10000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        if Config.IllegalMedical.Minigame.Enabled == true then
            if Config.IllegalMedical.Minigame.PSUI then
                exports['ps-ui']:Circle(function(success)
                    if success then
                        TriggerServerEvent("psv-grandma:server:MedicalAidInjuries")
                    else
                        ConstantDevelopmentGrandma(3, "You failed my Challenge and expect me to help you? Are you insane...", Config.IllegalMedical.OkOkNotificationTitle)
                    end
