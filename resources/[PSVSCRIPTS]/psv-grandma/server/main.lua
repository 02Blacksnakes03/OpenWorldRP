...
RegisterNetEvent('psv-grandma:server:hug', function(coords)
    TriggerClientEvent("psv-grandma:client:hug", -1, coords)
end)

RegisterNetEvent('psv-grandma:server:idle', function(coords)
    TriggerClientEvent("psv-grandma:client:idle", -1, coords)
end)

RegisterNetEvent('psv-grandma:server:MedicalAid', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('psv-grandma:server:MedicalAidInjuries', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local HasCash = Player.Functions.GetMoney("cash")
    local HasBank = Player.Functions.GetMoney("bank")
