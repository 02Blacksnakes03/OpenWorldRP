...
function closeQuestion(passed) {
	$.post('https://psv_flightschool/action', JSON.stringify({
		action: 'close',
		passed: passed
	}));
}
...
$(".theory-next").click(function(e) {
	...
	if ($('input[name="question"]:checked').val()) {
	  	if(questionNumber != QuestionToAnswer[selectedCategory]){
			...
			questionNumber++;
			openQuestion()
			$.post('https://psv_flightschool/action', JSON.stringify({action: 'playSound', type: 'nextQuestion'}));
		} else {
			answers.push($('input[name="question"]:checked').val());
			var nbGoodAnswer = 0;
			...
			if (nbGoodAnswer >= NeedAnswersToPass[selectedCategory]) {
				...
				$.post('https://psv_flightschool/action', JSON.stringify({action: 'playSound', type: 'passedExam'}));
			} else {
				...
				$.post('https://psv_flightschool/action', JSON.stringify({action: 'playSound', type: 'failedExam'}));
			}
		}
		return false;
	}
