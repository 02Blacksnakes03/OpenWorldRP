...
RegisterNetEvent('psv_flightschool:sv:buyExam', function(data)
    local src = source
        ...
        if Config.Core == "ESX" then
            ...
            if xPlayer then
                ...
                if myMoney >= examPrice then
                    ...
                    TriggerClientEvent('psv_flightschool:cl:startExam', src, data.type, data.category)
                else
                    if Config.PossibleChargeByBank then
                        ...
                        if myBank >= examPrice then
                            ...
                            TriggerClientEvent('psv_flightschool:notification', src, Config.Translate['not_enough_cash_and_bank'], 'error', true)
                        end
                    else
                        TriggerClientEvent('psv_flightschool:notification', src, Config.Translate['not_enough_cash'], 'error', true)
                    end
                end
            end
        elseif Config.Core == "QB-Core" then
