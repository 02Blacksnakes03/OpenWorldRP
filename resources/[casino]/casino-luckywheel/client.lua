...
CreateThread(function()
    ...
    LuckyWheelZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            text = "<b>Ein kostenloser Dreh alle 24 Stunden</b>"
...
RegisterNetEvent('nchub::casinoLuckyWheelHeader', function()
    exports['qb-menu']:showHeader({
        {
            header = "Lucky Wheel im Diamond Casino & Resort",
            isMenuHeader = true,
        },
        {
            header = "Versuche dein Glück",
            txt = "Ein kostenloser Dreh alle 24 Stunden",
            params = {
                event = "luckywheel:client:startWheel",
            }
        },
...
-- 		else
-- 			QBCore.Functions.Notify('Du bist kein VIP des Casinos', 'error', 3500)
-- 		end
-- 	end, "casino_vip")
-- end)
...
RegisterNetEvent("luckywheel:client:startWheel", function()
    QBCore.Functions.TriggerCallback('nchub::server:HasVIPMembership', function(HasItem)
        if HasItem then
            ...
            QBCore.Functions.Notify('Du hast keine VIP-Mitgliedschaft!', 'error', 3500)
            text = '<b>Das Diamond Casino & Resort</p>Bitte besuche die Rezeption!</b>'
            exports['qb-core']:DrawText(text)
        end
    end)
end)
