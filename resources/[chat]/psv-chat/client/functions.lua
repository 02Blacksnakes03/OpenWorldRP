...
function Koci.Client:TriggerServerCallback(key, payload, func)
    ...
    TriggerServerEvent("psv-chat:Server:HandleCallback", key, payload)
end
...
function onUpdateAnyNameValue()
    Koci.Client:TriggerServerCallback("psv-chat:updateNameValue", Koci.Client.Name)
end

function onUpdateMeDoPP()
    Koci.Client:TriggerServerCallback("psv-chat:updateMeDoPP", Koci.Client.MeDoPP)
end
...
function Koci.Client:LearnPlayerBalance()
    local xPlayer = self:GetPlayerData()
    if xPlayer then
        self:TriggerServerCallback("psv-chat:sendBalanceInfoMessage")
    end
end
...
function Koci.Client:RollTheDice()
    ...
    if xPlayer then
        ...
        self:TriggerServerCallback("psv-chat:RollTheDice", {
            diceValue = diceValue
        })
    end
end
...
function Koci.Client:RPS()
    ...
    if xPlayer then
        ...
        Citizen.Wait(1500)
        ClearPedTasks(PlayerPedId())
        self:TriggerServerCallback("psv-chat:RPS", {
            rpsValue = rpsValue
        })
    end
end
