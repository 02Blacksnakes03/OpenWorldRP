...
AddEventHandler("playerDropped", function()
    ...
    if gMusics[src] then
        gMusics[src] = false
        TriggerClientEvent("psv-chat:DestroyMusic", src, src)
    end
end)

Koci.Server:RegisterServerCallback("psv-chat:getCommands", function(source, data, cb)
    local commandList = {}
...
Koci.Server:RegisterServerCallback("psv-chat:registerRPText", function(source, data, cb)
    local src = source
    ...
    for _, player in ipairs(GetPlayers()) do
        if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(player))) < 10 then
            TriggerClientEvent("psv-chat:registerRPText", player, {
                source = src,
                type = data.type,
                text = data.text,
                meDoPP = _meDoPP,
...
Koci.Server:RegisterServerCallback("psv-chat:RollTheDice", function(source, data, cb)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
...
Koci.Server:RegisterServerCallback("psv-chat:RPS", function(source, data, cb)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
...
Koci.Server:RegisterServerCallback("psv-chat:showIdentity", function(source, data, cb)
    for _, player in ipairs(GetPlayers()) do
...
Koci.Server:RegisterServerCallback("psv-chat:sendJobMessages", function(source, data, cb)
...
Koci.Server:RegisterServerCallback("psv-chat:getCharacterInfo", function(source, data, cb)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    local _PlayerData = {
...
Koci.Server:RegisterServerCallback("psv-chat:sendWhisperMessage", function(source, data, cb)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
...
Koci.Server:RegisterServerCallback("psv-chat:sendMessages", function(source, data, cb)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
...
Koci.Server:RegisterServerCallback("psv-chat:sendYellowPages", function(source, data, cb)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
...
Koci.Server:RegisterServerCallback("psv-chat:sendAdminMessages", function(source, data, cb)
    local src = source
...
Koci.Server:RegisterServerCallback("psv-chat:sendAdminAnnounce", function(source, data, cb)
    local src = source
    if IsPlayerAceAllowed(src, "command") then
        for _, player in ipairs(GetPlayers()) do
            TriggerClientEvent("chat:addMessage", player, {
...
Koci.Server:RegisterServerCallback("psv-chat:playMusicWithUrl", function(source, data)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    local data = {
        url = data.url,
    ...
    if xPlayer and data.url then
        ...
        gMusics[src] = data
        TriggerClientEvent("psv-chat:SynchronizeMusics", src, data, src)
    else
...
Koci.Server:RegisterServerCallback("psv-chat:stopMusic", function(source)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    if xPlayer then
        if gMusics[src] then
            gMusics[src] = nil
            TriggerClientEvent("psv-chat:DestroyMusic", -1, src)
        end
    end
end)

Koci.Server:RegisterServerCallback("psv-chat:setMusicVolume", function(source, data)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    if xPlayer then
        if gMusics[src] then
            ...
            TriggerClientEvent("psv-chat:SynchronizeMusics", src, gMusics[src], src)
        end
    end
end)

Koci.Server:RegisterServerCallback("psv-chat:updateNameValue", function(source, data)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    if xPlayer then
...
Koci.Server:RegisterServerCallback("psv-chat:updateMeDoPP", function(source, data)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    if xPlayer then
        if Koci.Server.Players[src] then
...
Koci.Server:RegisterServerCallback("psv-chat:sendBalanceInfoMessage", function(source)
    local src = source
    local xPlayer = Koci.Server:GetPlayer(src)
    if xPlayer then
        ...
        TriggerClientEvent("psv-chat:BalanceInfoMessage", src, {
            sourceId = src,
            header = PlayerName,
            balance = {
                bank = bankBalance,
...
RegisterNetEvent("psv-chat:Server:HandleCallback", function(key, payload)
    local src = source
    if Koci.Callbacks[key] then
        Koci.Callbacks[key](src, payload, function(cb)
            TriggerClientEvent("psv-chat:Client:HandleCallback", src, key, cb)
        end)
    end
end)
