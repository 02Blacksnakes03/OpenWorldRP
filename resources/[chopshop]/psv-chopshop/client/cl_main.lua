...
Citizen.CreateThread(function()
    while true do
        ...
        if scrapblip then
            ...
            if not mert then
                print(scanner)
                exports['psv-chopshop']:SetupDigiScanner(scanner, {})
                mert = true
            end
            if dist <= 3 then
                if not copsCalled then
                    ...
                    TriggerServerEvent("psv-chopshop:server:callCops", "Chopshop", 0, streetLabel, pos)
                    copsCalled = true
                end
                Citizen.Wait(5000)
                QBCore.Functions.Notify(Config.Locale["FoundVeh"], 'success')
                exports["psv-ui"]:Show(Config.Locale["title"], Config.Locale["chop3"])
                Citizen.Wait(10000)
...
RegisterNetEvent('psv-chopshop:jobaccept')
AddEventHandler('psv-chopshop:jobaccept', function()
...
function SpawnVehicle(vehicle, x, y, z, w)
    ...
    QBCore.Functions.SpawnVehicle(vehicle, function(veh)
        ...
        exports["psv-ui"]:Show(Config.Locale["title"], Config.Locale["chop1"])
        ...
        if Config.Email == true then
            ...
            exports["psv-ui"]:Show(Config.Locale["title"],
...
function ScrapVehicle()
    ...
    if GetVehicleNumberPlateText(veh) ~= LicensePlate then
        ...
        exports["psv-ui"]:Show(Config.Locale["title"], Config.Locale["chop4"])
        StartChopping()
        DeleteBlip()
    end
end
...
RegisterNetEvent('psv-chopshop:client:robberyCall')
AddEventHandler('psv-chopshop:client:robberyCall', function(type, key, streetLabel, coords)
...
function StartAnimation(k)
    if Config.CarTable[k].anim == "wheel1" or Config.CarTable[k].anim == "wheel2" or Config.CarTable[k].anim == "wheel3" or Config.CarTable[k].anim == "wheel4" then
        TriggerEvent('psv-chopshop:wheelanimation')
        Citizen.Wait(7000)
        ...
        TriggerServerEvent('psv-chopshop:server:rewardplayer', Config.CarTable[k].anim)
    elseif Config.CarTable[k].anim == "door" then
        TaskOpenVehicleDoor(GetPlayerPed(-1), vehicle, 3000, Config.CarTable[k].getin, 10)
        Citizen.Wait(2500)
        TriggerEvent('psv-chopshop:dooranimation')
        Citizen.Wait(4200)
    ...
    elseif Config.CarTable[k].anim == "trunk" then
        ...
        TriggerEvent('psv-chopshop:trunkanimation')
        Citizen.Wait(2000)
        Config.CarTable[k].chopped = true
        TriggerServerEvent('psv-chopshop:server:rewardplayer', Config.CarTable[k].anim)
        Citizen.Wait(1000)
    ...
    elseif Config.CarTable[k].anim == "hood" then
        SetVehicleDoorOpen(vehicle, Config.CarTable[k].destroy, false, true)
        TriggerEvent('psv-chopshop:hoodanimation')
        Citizen.Wait(4000)
        SetVehicleDoorBroken(vehicle, Config.CarTable[k].destroy, true)
        Config.CarTable[k].chopped = true
        TriggerServerEvent('psv-chopshop:server:rewardplayer', Config.CarTable[k].anim)
