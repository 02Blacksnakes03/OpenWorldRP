...
RegisterNetEvent('psv-rental:createCar')
AddEventHandler('psv-rental:createCar', function(vehicle,model,plate,day)
...
AddEventHandler('onResourceStart', function(resourceName)
    if (GetCurrentResourceName() == resourceName) then 
        if ped ~= nil then
            ...
            TriggerEvent('psv-rental:createPed')
            MySQL.Async.execute("DELETE FROM player_vehicles WHERE datefinish < NOW()", {})
            return
        end
end)

QBCore.Functions.CreateCallback('psv-rental:rent', function(source,cb,data)
    local src = source
