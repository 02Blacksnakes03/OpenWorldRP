                ...
                var confirmIcon = $("<img>", {
                    ...
                    click: function() {
                        ...
                        if (!isNaN(numericValue) && numericValue.toString() === inputValue && numericValue >= 1) {
                            $.post('https://psv-selldrugs/dialogBack', JSON.stringify({ action: "sell", itemName: selectedItemName, inputValue: numericValue, isWholesale: false }))
        ...
        if (!isTyping) {
            ...
            if (response.action !== null) {
                ...
                if (response.endInteraction !== null && response.endInteraction) {
                    $.post('https://psv-selldrugs/successInteraction', JSON.stringify({ itemName: itemToSellName, amount: npcQuantity[itemToSellName], price: sellPriceValue }))
                } else {
                    $.post('https://psv-selldrugs/dialogBack', JSON.stringify({ action: response.action }))
                }
