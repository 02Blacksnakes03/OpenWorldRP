...
function togglePhoneUI(show) {
    if (show) {
        ...
        $.post('https://psv-selldrugs/phoneExit');
    }
}
    ...
    callAudio.onended = function() {
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                $.post('https://psv-selldrugs/phoneStartTradeMission', JSON.stringify({ item:  conversations[conversationId].itemName, quantity: quantity, price: conversations[conversationId].price }) )
    ...
    conversation.messages.forEach(function(message, index) {
        ...
        if (index == conversation.messages.length - 1 && conversation.gps) {
            ...
            $image.click(function() {
                ...
                $.post('https://psv-selldrugs/setWaypoint', JSON.stringify({ coords: gpsCoords }))
            });
        }
...
function inputAccept(type, inputField, conversationId, conversation) {
    ...
    if (!isNaN(numericValue) && numericValue.toString() === inputValue && numericValue >= 1) {
        if (type == 'amount') {
            ...
            var randomDelay = Math.floor(Math.random() * (5500 - 3000 + 1)) + 3000;
            $.post('https://psv-selldrugs/dialogBack', JSON.stringify({ action: "sell", itemName: conversation.itemName, inputValue: inputNumber, isWholesale: true }))
                .done(function(data) {
                    if (data.result) {
                        setTimeout(() => {
...
$('#notificationSwitch').change(function() {
    if (canChangeSwitch(lastNotificationSwitchTime)) {
        ...
        $.post('https://psv-selldrugs/saveSettings', JSON.stringify({ statusAlerts: statusAlerts, statusAlertsSound: statusAlertsSound }));
    } else {
