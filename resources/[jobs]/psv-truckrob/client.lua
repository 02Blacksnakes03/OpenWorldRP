...
exports["qb-target"]:AddCircleZone("BankTruckNPC", vector3(-1048.614, -524.0357, 36.588905), 0.6,
    {
        name = "BankTruckNPC",
        debugPoly = false,
        useZ = true
    },
    {
        options = {
            {
                event = "psv-truckrob:RobberyCheck",
                icon = "fas fa-truck",
                label = "Use Calling Card",
            },
        },
        distance = 1.5
    }
...
RegisterNetEvent('psv-truckrob:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('psv-truckrob:CustomNotify',message, type)
                    ...
                    if Dist <= 50 then
                        ...
                        if Dist <= 3 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('psv-truckrob:RobberyCheck')
                            end
                        end
                    end
                end
    ...
    if Config.UsePed then
        ...
        while true do
            ...
            if Dist <= 50 and not PedSpawned then
                TriggerEvent('psv-truckrob:SpawnNPC',Config.StartPed,Config.StartModel)
                PedSpawned = true
...
RegisterNetEvent('psv-truckrob:SpawnNPC',function(coords,model)
...
RegisterNetEvent('psv-truckrob:RobberyCheck', function()
    if not Config.AllowLeoRob and isLawEnforcement then
        TriggerEvent('psv-truckrob:Notify', Config.Lang['nocop'], Config.LangType['info'])
        return
    end
    if onjob then
        TriggerEvent('psv-truckrob:Notify', Config.Lang['onjob'], Config.LangType['info'])
        return
    end
    local StartRobbery = true
    local hasItem = true
    TriggerEvent('psv-truckrob:Notify', Config.Lang['checking'], Config.LangType['info'])
    if Config.RequireMinimumLEO then
        StartRobbery = false
        if Config.UseESX then
            ESX.TriggerServerCallback('psv-truckrob:PoliceAvailable:ESX', function(cb)
                StartRobbery = cb
            end)                                    
        elseif Config.UseQBCore then
            QBCore.Functions.TriggerCallback('psv-truckrob:PoliceAvailable:QBCore', function(cb)
                StartRobbery = cb
            end)
        end
        Wait(1000)
    ...
    if not StartRobbery then
        TriggerEvent('psv-truckrob:Notify', Config.Lang['mincops'], Config.LangType['error'])
        return
    end
    if Config.RequireStartItem then
        hasItem = false
        if Config.UseESX then
            ESX.TriggerServerCallback('psv-truckrob:itemTaken:ESX', function(cb)
                hasItem = cb
            end)                                    
        elseif Config.UseQBCore then
            QBCore.Functions.TriggerCallback('psv-truckrob:itemTaken:QBCore', function(cb)
                hasItem = cb
            end)
        end
        Wait(1000)
    ...
    if hasItem then
        onjob = true
        TriggerServerEvent('psv-truckrob:Server:Guards')
        TriggerEvent('psv-truckrob:Notify', Config.Lang['startHeist'], Config.LangType['info'])
    else
        TriggerEvent('psv-truckrob:Notify', Config.Lang['noitem'], Config.LangType['error'])
    end
end)

RegisterNetEvent('psv-truckrob:Client:HesitSyncStarterStart', function(finish, start)
    BlipAdder(start, finish)
end)

RegisterNetEvent('psv-truckrob:Client:HesitSyncCopsStart', function(finish, start)
    if isLawEnforcement then
        CreateThread(function()
            BlipAdder(start, finish)
        end)
        TriggerEvent('psv-truckrob:Notify', Config.Lang['cop_notify'], Config.LangType['info'])
    end
end)

RegisterNetEvent('psv-truckrob:Client:BeginRoute', function(finish, time, start, list)
    GaurdList[time] = {}
    ...
    TriggerEvent('psv-truckrob:PoliceAlert', GetEntityCoords(vehicle))
    TriggerServerEvent('psv-truckrob:Server:TruckSync', finish, time, start, id)
    onjob = false
...
RegisterNetEvent('psv-truckrob:Client:TruckSync', function(time, inital, final, netid)
        ...
        if Config.UseThirdEye then
            if Config.ThirdEyeName == 'ox_target' then
                local options = {
                    {
                        onSelect = function()
                            TriggerServerEvent('psv-truckrob:Server:ArmExplosion', obj, time)
                            LootAnim(obj)
                        end,
                        icon = 'fas fa-truck',
                    ...
                    {
                        onSelect = function()
                            TriggerEvent('psv-truckrob:Loot', obj, netid, time)
                        end,
    ...
    while true do 
        ...
        if #(Pos-truckPos) <= 350 and DoesEntityExist(obj) then
            active = true
            TriggerEvent('psv-truckrob:Notify', Config.Lang['truck_ping'], Config.LangType['info'])
            break
...
RegisterNetEvent('psv-truckrob:Client:ArmExplosion', function(obj, number)
    EPlant[number] = true
...
RegisterNetEvent('psv-truckrob:Loot', function(obj, netid, time)
    if not DoesEntityExist(obj) then return end
    TriggerServerEvent('psv-truckrob:Server:ThirdEyeSync', netid, time)
    LootAnim(obj)
    TriggerServerEvent('psv-truckrob:Server:HeistReward')
end)

RegisterNetEvent('psv-truckrob:Client:ThirdEyeSync', function(netid, time)
    ELoot[time] = true
    local obj = NetworkGetEntityFromNetworkId(netid)
    while not DoesEntityExist(obj) do
        if DoesEntityExist(obj) then
...
function TruckSpawner(time, start, finsh, truck)
    ...
    if limit <= 0 then TriggerEvent('psv-truckrob:Notify', src, Config.Lang['time_limit'], Config.LangType['error']) return false end
