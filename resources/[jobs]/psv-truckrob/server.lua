if Config.UseESX then
    ...
    ESX.RegisterServerCallback('psv-truckrob:PoliceAvailable:ESX',function(source,cb)
    ...
    ESX.RegisterServerCallback('psv-truckrob:itemTaken:ESX',function(source,cb)
...
elseif Config.UseQBCore then
    ...
    QBCore.Functions.CreateCallback('psv-truckrob:PoliceAvailable:QBCore', function(source, cb)
        local cops = 0
    ...
    QBCore.Functions.CreateCallback('psv-truckrob:itemTaken:QBCore', function(source, cb)
...
RegisterNetEvent('psv-truckrob:Server:HeistReward', function()
	local src = source
    local type = nil
	local Player = nil
    ...
    if math.random(1, 100) <= Config.RareLootChance then
        ...
        TriggerClientEvent('psv-truckrob:Notify', src, Config.Lang['gained_rare'], Config.LangType['success'])
    end
end)
--- Guard Spawner
RegisterNetEvent('psv-truckrob:Server:Guards', function()
    local List = {}
    local time = os.time()
    ...
    TriggerClientEvent("psv-truckrob:Client:HesitSyncStarterStart", source, nil, route)
    TriggerClientEvent("psv-truckrob:Client:HesitSyncCopsStart", -1, nil, route)
    TriggerClientEvent("psv-truckrob:Client:BeginRoute", source, nil, time, route, List)
end)

-- Syncs
RegisterNetEvent('psv-truckrob:Server:TruckSync', function(finish, time, start, id)
    TriggerClientEvent("psv-truckrob:Client:TruckSync", -1, time, start, finish, id)
end)

RegisterServerEvent('psv-truckrob:Server:ThirdEyeSync', function(name, time)
    TriggerClientEvent('psv-truckrob:Client:ThirdEyeSync', -1, name, time)
end)

RegisterServerEvent('psv-truckrob:Server:ArmExplosion', function(name, time)
    TriggerClientEvent('psv-truckrob:Client:ArmExplosion', -1, name, time)
end)
...
-- Exploit Trigger
RegisterServerEvent('psv-truckrob:ThatIsAThing', function(server)
    if server ~= nil then
        DropPlayer(server, "Go hack somewhere else.")
    end
    Print("\n\n\n\nWARNING WARNING WARNING\nPlayer ID "..tostring(server).." was kicked for attempting to exploit psv-truckrob. It is recommended you ban them.\nnWARNING WARNING WARNING\n\n\n\n")
end)
