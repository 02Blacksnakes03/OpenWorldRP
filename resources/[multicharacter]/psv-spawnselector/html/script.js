...
$(document).on('click', '#IconClassStyle', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('https://psv-spawnselector/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '.GreenBTN', function(evt){
    ...
    if (Mainlocation !== null){
        ...
        if (MainspawnType == "appartment2") {
            $.post('https://psv-spawnselector/spawnplayerappartment2', JSON.stringify({
                spawnloc: Mainlocation,
                apartName: apartNames,
            }));
        } else if(MainspawnType !== "appartment"){
            $.post('https://psv-spawnselector/spawnplayer', JSON.stringify({
                spawnloc: Mainlocation,
                typeLoc: MainspawnType
            }));
        }else {
            $.post('https://psv-spawnselector/chooseAppa', JSON.stringify({
...
function setupNewLocations(locations, myHouses, Apartment, ApartmentName, Access) {
    ...
    setTimeout(function(){
        ...
        $.each(locations, function(index, location){
            ...
            $(parent).append('<i style="top:'+location.pos.top+'%; left:'+location.pos.left+'%; font-size: 35px; color: #333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); letter-spacing: 1px; line-height: 1; padding: 1px 3px; border-radius: 5px; background-color: #fff; border: 1px solid #ccc;" data-location="'+location.location+'" data-type="normal" data-label="'+location.label+'" id="IconClassStyle" class="IconClassStyle">&#9670;</i>');
        });
...
function CloseAddCoord() {
    $.post('https://psv-spawnselector/CloseAddCoord', JSON.stringify({}));
