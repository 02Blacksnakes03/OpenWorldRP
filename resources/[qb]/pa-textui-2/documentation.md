...
CreateThread(function()
    ...
    exports['pa-textui-2']:delete3DTextUI("pa-test")
    -- For Players
    exports['pa-textui-2']:create3DTextUIOnPlayers("pa-test", {
        id = targetId,
        ped = targetPed,
        displayDist = 6.0,
        interactDist = 2.0,
    ...
    exports['pa-textui-2']:delete3DTextUIOnPlayers(targetId)
...
```lua
    ...
    local function drawText(text, position)
        ...
        exports["pa-textui-2"]:displayTextUI(text, position)
    end

    local function changeText(text, position)
        if type(position) ~= 'string' then position = 'left' end
        exports['pa-textui-2']:changeText(text, position)
    end
...
```lua
    function ESX.TextUI(message, notifyType)
        if GetResourceState("pa-textui-2") ~= "missing" then
            return exports["pa-textui-2"]:displayTextUI(message, notifyType)
        end
        print("[^1ERROR^7] ^5pa-textui-2^7 is Missing!")
    end

    function ESX.HideUI()
        if GetResourceState("pa-textui-2") ~= "missing" then
            return exports["pa-textui-2"]:hideTextUI()
        end
        print("[^1ERROR^7] ^5pa-textui-2^7 is Missing!")
    end
```