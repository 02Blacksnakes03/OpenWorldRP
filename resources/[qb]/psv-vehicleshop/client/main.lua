...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.VehicleShops) do
        ...
        if v.Management.Enable then
            exports['interact']:create3DTextUI("psv-vehicleshop-vehshop-" .. k, {
                ...
                triggerData = {
                    triggerName = "psv-vehicleshop:openSellingMenu:client",
...
RegisterNetEvent('psv-vehicleshop:openSellingMenu:client', function(data)
    sellMenuActive = data.open
    if sellMenuActive then
        local showAddStockBtn = false
        TriggerCallback('psv-vehicleshop:checkIsPlayerHasPerm:server', function(hasPerm)
            if hasPerm then
        ...
        if Config.VehicleShops[data.vehshopId].EnableStocks then
            ...
            TriggerCallback('psv-vehicleshop:getVehStock:server', function(stockData)
                if stockData ~= 0 then
...
--     Initialized = true
--     Citizen.CreateThread(function()
--         TriggerCallback('psv-vehicleshop:getShowroomData:server', function(showroomData)
...
--                     exports['interact']:create3DTextUI("psv-vehicleshop-showroom-vehicle-" .. model .. "-spotId-" .. v.spotId, {
...
--                         theme = "green", 
--                         triggerData = {
--                             triggerName = "psv-vehicleshop:askAnEmployee:client",
--                             args = {}
--                         }
--                     })
--                 end
...
RegisterNetEvent('psv-vehicleshop:askAnEmployee:client', function()
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "close" then
        ...
        elseif data.type == "showroomEdit" then
            ...
            TriggerServerEvent('psv-vehicleshop:updateShowroomVehicles:server', data.dealershipId, Config.VehicleShops[data.dealershipId].ShowroomVehicles)
        elseif data.type == "sellMenu" then
            TriggerEvent('psv-vehicleshop:openSellingMenu:client', {open = false})
        else
            SetNuiFocus(false, false)
        end
        if data.test == true and not testDriveState then
            if IsSpawnPointClear(vector3(currentCoords.testVehCoords.x, currentCoords.testVehCoords.y, currentCoords.testVehCoords.z), 2.5) then
                ...
                TriggerServerEvent('psv-vehicleshop:startTest:server', currentTestVehicleNetId)
    ...
    elseif data.action == "updateShowroomVehicle" then
        ...
        --TriggerServerEvent('psv-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model, props)
        TriggerServerEvent('psv-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model)
    ...
    elseif data.action == "createTextsOnPlayers" then
        ...
        if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
            ...
            for _, id in pairs(nearbyPlayers) do
                exports['interact']:create3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-" .. id, {
                    id = id,
                    displayDist = 5.0,
                    interactDist = 1.3,
                    ...
                    theme = "green", -- or red
                    triggerData = {
                        triggerName = "psv-vehicleshop:sendSellRequest:client",
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                for _, id in pairs(nearbyPlayers) do
                    exports['interact']:delete3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-" .. id)
                end
            end
        else
    ...
    elseif data.action == "confirmPurchase" then
        TriggerServerEvent('psv-vehicleshop:buyVehicle:server', data.paymentType, data.model, Vehicles2[data.model].price, data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
    elseif data.action == "addStockForAllVehicles" then
        ...
        TriggerServerEvent('psv-vehicleshop:updateDealershipStockData:server', data.dealershipId, stocks)
        taskActive = false
...
RegisterNetEvent('psv-vehicleshop:sendSellRequest:client', function(data)
    -- Delete All
    if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
        for _, id in pairs(nearbyPlayers) do
            exports['interact']:delete3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-" .. id)
    ...
    -- Add To Sender
    TriggerServerEvent('psv-vehicleshop:sendRequestText:server', data.sender, data.target, data.price, data.model, data.dealershipId)
    Citizen.Wait(500)
    taskActive = false
end)

RegisterNetEvent('psv-vehicleshop:sendRequestText:client', function(sender, price, model, dealershipId)
    exports['interact']:create3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-confirm-purchase-" .. sender, {
        id = sender,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true,
    ...
    taskActive = false
    Notify("Request timed out.", 7500, "error")
    exports['interact']:delete3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-confirm-purchase-" .. sender)
end)

RegisterNetEvent('psv-vehicleshop:acceptPayment:client', function(data)
    Notify("Request accepted.", 7500, "success")
    exports['interact']:delete3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('psv-vehicleshop:buyVehicle:server', "bank", data.model, tonumber(data.price), data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
end)

RegisterNetEvent('psv-vehicleshop:declinePayment:client', function(data)
    Notify("Request declined.", 7500, "error")
    exports['interact']:delete3DTextUIOnPlayers("psv-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('psv-vehicleshop:declinePayment:server', data.sender)
end)
...
AddEventHandler('onResourceStop', function(resource)
    if GetCurrentResourceName() == resource then
        ...
        for k, v in pairs(Config.VehicleShops) do
            ...
            exports['interact']:delete3DTextUI("psv-vehicleshop-vehshop-" .. k)
        end
    end
end)
...
RegisterNetEvent('psv-vehicleshop:buyVehicle:client', function(model, plate, dealershipId)
    DoScreenFadeOut(500)
    openDealership(false)
    Citizen.Wait(1000)
...
RegisterNetEvent('psv-vehicleshop:finishTest:client', function()
    testDriveState = false
...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == "CEventNetworkVehicleUndrivable" then
        ...
        if netId == currentTestVehicleNetId then
            TriggerEvent('psv-vehicleshop:finishTest:client')
            TriggerServerEvent('psv-vehicleshop:finishTest:server', netId)
        end
    end
                ...
                for k, v in pairs(Config.VehicleShops) do
                    ...
                    if dist <= 3 then
                        function currentShow()
                            exports["interact"]:displayTextUI("Give Back Test Vehicle", "E")
                            showTextUI = true
                        end
                        function currentHide()
                            showTextUI = false
                            exports["interact"]:hideTextUI()
                        end
				...
				if closestDeliveryPoints.distance < closestDeliveryPoints.maxDist then
					if IsControlJustReleased(0, 38) then
                        ...
                        if VehToNet(veh) == VehToNet(currentTestVehicle) then
                            testDriveState = false
                            SendNUIMessage({action = "stopTestTimer"})
                            TriggerServerEvent('psv-vehicleshop:finishTest:server', VehToNet(currentTestVehicle))
...
RegisterNetEvent('psv-vehicleshop:deleteVehicle:client', function(netId)
...
--RegisterNetEvent('psv-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('psv-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, createNew)
    -- Delete Old
