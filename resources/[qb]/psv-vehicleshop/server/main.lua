...
local StringCharset = {}
local NumberCharset = {}
RegisterNetEvent('psv-vehicleshop:buyVehicle:server', function(type, vehicle, price, dealershipId, sender, job)
    local src = source
    ...
    TriggerClientEvent('psv-vehicleshop:buyVehicle:client', src, vehicle, plate, dealershipId)
...
CreateCallback('psv-vehicleshop:generatePlate:server', function(source, cb)
    local plate = GeneratePlate()
    cb(plate)
end)
...
-- Stocks
RegisterNetEvent('psv-vehicleshop:updateDealershipStockData:server', function(dealershipId, stocksData)
    local dealershipData = MySQL.query.await('SELECT * FROM pa_vehicleshop_stocks WHERE dealershipId = ?', {dealershipId})
...
CreateCallback('psv-vehicleshop:getVehStock:server', function(source, cb, dealershipId)
    local dealershipData = MySQL.query.await('SELECT * FROM pa_vehicleshop_stocks WHERE dealershipId = ?', {dealershipId})
    local anusVal = {}
    for k, v in pairs(dealershipData) do
        for a, b in pairs(v) do
...
local testVehicles = {}
RegisterNetEvent('psv-vehicleshop:startTest:server')
AddEventHandler('psv-vehicleshop:startTest:server', function(netId)
    testVehicles[netId] = {
        playerId = source
    }
end)

RegisterNetEvent('psv-vehicleshop:finishTest:server')
AddEventHandler('psv-vehicleshop:finishTest:server', function(netId)
...
AddEventHandler('entityRemoved', function(entity)
    ...
    if testVehicles[netId] then
        TriggerClientEvent('psv-vehicleshop:finishTest:client', testVehicles[netId].playerId)
        testVehicles[netId] = nil
    end
end)

RegisterNetEvent('psv-vehicleshop:updateShowroomVehicles:server', function(dealershipId, data)
    local src = source
...
CreateCallback('psv-vehicleshop:getShowroomData:server', function(source, cb, dealershipId)
    local showroomTable = {}
...
RegisterNetEvent('psv-vehicleshop:sendRequestText:server', function(sender, target, price, model, dealershipId)
    TriggerClientEvent('psv-vehicleshop:sendRequestText:client', target, sender, price, model, dealershipId)
end)

RegisterNetEvent('psv-vehicleshop:declinePayment:server', function(sender)
...
AddEventHandler('playerDropped', function()
    for k, v in pairs(testVehicles) do
        if v.playerId == source then
            TriggerClientEvent('psv-vehicleshop:deleteVehicle:client', -1, k)
        end
    end
end)

--RegisterNetEvent('psv-vehicleshop:deleteVehicleShowroom:server', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('psv-vehicleshop:deleteVehicleShowroom:server', function(dealershipId, spotId, newModel)
    for _, playerId in ipairs(GetPlayers()) do
        ...
        if numPlayerId ~= source then
            ...
            if dist <= 40 then
                --TriggerClientEvent('psv-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, props)
                TriggerClientEvent('psv-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, true)
            else
                TriggerClientEvent('psv-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, false)
            end
        end
    end
end)

CreateCallback('psv-vehicleshop:checkIsPlayerHasPerm:server', function(source, cb)
