    ...
    function self.Functions.RemoveItem(item, amount, slot)
        ...
        if slot then
            if self.PlayerData.items[slot].amount > amount then
                ...
                if not self.Offline then
                    self.Functions.UpdatePlayerData()
                    TriggerEvent('QBCore:OnRemoveInventoryItem', self.PlayerData.source, item, self.PlayerData.items[slot].amount)
            ...
            elseif self.PlayerData.items[slot].amount == amount then
                ...
                if not self.Offline then
                    self.Functions.UpdatePlayerData()
                    TriggerEvent('QBCore:OnRemoveInventoryItem', self.PlayerData.source, item, 0)
            ...
            if slots then
                for _, _slot in pairs(slots) do
                    if self.PlayerData.items[_slot].amount > amountToRemove then
                        ...
                        if not self.Offline then
                            self.Functions.UpdatePlayerData()
                            TriggerEvent('QBCore:OnRemoveInventoryItem', self.PlayerData.source, item, self.PlayerData.items[_slot].amount)
