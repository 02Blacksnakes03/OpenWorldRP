...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/emotemenu', 'Open dpemotes menu (F4) by default. This may differ from server to server.')
    ...
    if #args > 0 then
        if IsEntityDead(PlayerPedId()) then
            TriggerEvent('chat:addMessage', {
                color = {255, 0, 0},
                multiline = true,
                args = {"dpemotes", Config.Languages[lang]['dead']}
            })
            return
        end
        if (IsPedSwimming(PlayerPedId()) or IsPedSwimmingUnderWater(PlayerPedId())) and not Config.AllowInWater then
            TriggerEvent('chat:addMessage', {
                color = {255, 0, 0},
                multiline = true,
                args = {"dpemotes", Config.Languages[lang]['swimming']}
            })
            return
        end
