...
function RegisterUseableItem(name)
    ...
    if CoreName == "qb-core" or CoreName == "qbx_core" then
        Core.Functions.CreateUseableItem(name, function(source, item)
            TriggerClientEvent('ac-hacking:showIdCard:client', source, item.info or item.metadata)
        end)
    elseif CoreName == "es_extended" then
        local hasQs = GetResourceState('qs-inventory') == 'started'
        if hasQs then
            Core.RegisterUsableItem(name, function(source, item)
                TriggerClientEvent('ac-hacking:showIdCard:client', source, item.info)
            end)
            return
        end
        Core.RegisterUsableItem(k, function(source, _, item)
            TriggerClientEvent('ac-hacking:showIdCard:client', source, item.metadata)
        end)
    end
end
...
function TriggerCallback(name, source, cb, ...)
    if not AC.ServerCallbacks[name] then return end
    AC.ServerCallbacks[name](source, cb, ...)
end

RegisterNetEvent('ac-hacking:server:triggerCallback', function(name, ...)
    local src = source
    TriggerCallback(name, src, function(...)
        TriggerClientEvent('ac-hacking:client:triggerCallback', src, name, ...)
    end, ...)
end)